...
/* 优先使用系统中文字体，确保中文正常显示 */
body {
    font-family: "Heiti SC", "STHeiti", "Heiti", "PingFang SC", "Hiragino Sans GB", "WenQuanYi Micro Hei", "Noto Sans CJK SC", sans-serif;
    font-size: 11pt;
    line-height: 1.4;
    color: #333;
}
...
def render_pdf(
    ...
    try:
        ...
        # 添加中文字体 CSS
        chinese_font_css = CSS(string='''
            @font-face {
                font-family: 'Chinese Font';
                src: local('PingFang SC'), local('STHeiti'), local('SimHei'), 
                     local('Microsoft YaHei'), local('WenQuanYi Micro Hei'),
                     local('Noto Sans CJK SC');
            }
            body {
                font-family: 'Chinese Font', 'Heiti SC', 'STHeiti', 'Heiti', 
        ...
        html_doc.write_pdf(output_path, stylesheets=[chinese_font_css], 
                          font_config=font_config)
        logger.info("WeasyPrint rendered PDF to %s with Chinese font support", output_path)
    except ImportError:
