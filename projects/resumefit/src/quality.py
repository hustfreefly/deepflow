...
注：语义相似度已移除（Lite 版使用简单 Jaccard 导致误判，Full 版将引入 BGE-M3）
...
def _compute_semantic_similarity(resume_text: str, jd: JDSchema) -> float:
    ...
    Lite 版使用简单 Jaccard 相似度导致评分严重偏低（38.3），
    误导用户判断。Full 版将引入 BGE-M3 embedding 模型。
    
    当前固定返回 0.0，不参与任何评分计算。
    """
    return 0.0
...
def _generate_recommendations(metrics: QualityMetrics, jd: JDSchema, resume_text: str) -> List[str]:
    """生成改进建议。"""
    recommendations = []
    resume_words = set(_tokenize(resume_text))

    if metrics.keyword_coverage < 80:
        missing = [kw for kw in jd.keywords if kw.lower() not in resume_words]
        if missing:
...
def generate_quality_report(
    ...
    # JD 匹配评分（语义相似度已移除，直接使用关键词覆盖率）
    jd_match = round(keyword_coverage, 1)
    ...
    # 生成建议
    recommendations = _generate_recommendations(metrics, jd, resume_text)
