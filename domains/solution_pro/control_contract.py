...
def _normalize_experts(planning: Dict[str, Any], mode: str) -> List[Dict[str, str]]:
    ...
    for index, item in enumerate(raw_experts[:max_count], 1):
        ...
        normalized.append({
            ...
            "expected_output_path": _registry_path(f"research_{fixed_id}", f"stages/research_{fixed_id}.json"),
...
def _acceptance_from_frozen_spec(base: Path) -> List[Dict[str, str]]:
    """
    从 frozen_spec.json 生成 acceptance_criteria（V2.0 增强）。
    
    优先使用 requirement_groups 组织输出，如果没有则回退到扁平列表。
    """
    frozen = _load_json(base / "data" / "frozen_spec.json", {})
...
def build_control_contract(base_path: str) -> Dict[str, Any]:
    base = Path(base_path)
    plan = _load_json(base / "execution_plan.json", {})
    planning = _load_json(base / "stages" / "planning.json", {})
    ...
    return {
        ...
        "source": {
            "planner_output_path": "stages/planning.json",
        ...
        "acceptance_criteria": _acceptance_from_frozen_spec(base) or _normalize_acceptance_criteria(planning),
...
def rewrite_after_planning(base_path: str) -> Dict[str, Any]:
    ...
    contract_path = base / "control_contract.json"
    ...
    planning_path = str(base / "stages" / "planning.json")
    research_tasks = {}
    ...
    audit_path = str(base / "stages" / "audit.json")
    ...
    return {
        "status": "refreshed",
        "control_contract_path": str(contract_path),
