...
2. **Layer 2 四个检查项都做** — P0 覆盖、方案一致、Guardrails、信息守恒
...
### 软件域验证维度示例
```
检查维度：P0 需求覆盖、架构一致性、安全合规、性能指标
示例检查项：
- P0 REQ 是否 100% 覆盖？每个需求是否有对应实现？
- 架构设计（软件域参考）是否与约束一致？是否有语义矛盾？
- 安全约束是否全部满足？OWASP Top 10 是否缓解？
- 性能指标是否达标？响应时间 < 200ms？
```

### 投资域验证维度示例
```
检查维度：数据源覆盖、假设验证、风险维度覆盖、合规性
示例检查项：
- 关键财务数据是否有 3+ 独立数据源验证？
- 估值假设是否经敏感性分析验证？
- 技术/市场/监管三维度风险是否全面覆盖？
...
### 硬件域验证维度示例
```
检查维度：热设计裕量、可靠性指标、DFM 合规、BOM 成本
示例检查项：
- 热设计是否满足 TDP 要求？安全裕量是否足够？
- MTBF 是否达标？关键器件降额设计是否合规？
- DFM 评审是否通过？工艺能力 Cpk 是否 > 1.33？
- BOM 成本是否在目标内？供应链是否采用双源策略？
```
