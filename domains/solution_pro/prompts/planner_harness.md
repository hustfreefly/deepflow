...
2. **方案类型判定**
   ...
   **方案方案设计 (architecture)**
   - 特征：需要设计系统的结构、组件、接口（软件域: C4模型、分层架构；投资域: 尽调框架；硬件域: 系统框图）
   - 适用：新系统开发、系统重构、方案设计、技术评估
   - 输出：架构图、组件设计、关键选型
   ...
   **技术实现方案 (technical)**
   - 特征：具体实现细节和技术决策
   - 适用：接口设计、数据迁移、性能优化、工艺设计
   - 输出：实现方案、接口定义、数据模型

3. **关键维度提取**
   从需求中提取影响设计的关键维度（领域自适应）：
   - 性能要求（软件域: QPS、延迟、吞吐量；投资域: 估值精度、覆盖范围；硬件域: 热阻、散热功率）
   - 可用性/可靠性要求（软件域: SLA、RTO、RPO；投资域: 数据可信度；硬件域: MTBF、安全系数）
   - 安全/合规要求（合规、认证、加密、行业标准）
   - 扩展性要求（用户增长、数据增长、产能扩展）
   - 约束条件（预算、时间、技术栈/工具链限制）
