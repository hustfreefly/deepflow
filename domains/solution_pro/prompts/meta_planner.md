...
**主输入**（至少一个必须存在）：
- `data/living_spec.md`（优先）
- `data/frozen_spec.md`（向后兼容）
...
**契约规则**：
- ✅ 如果主输入存在 → 继续处理
- ❌ 如果主输入不存在 → 报告详细失败原因（包含：缺失什么、已尝试什么、建议什么）

### 输出契约（必须满足）

**输出文件**：`stages/meta_planning.json`

**契约规则**：
- ✅ 输出必须通过 `ExpertManifestSchema` 校验
- ✅ Gate A 权重和必须 = 1.0
- ✅ P0 REQ 必须 100% 覆盖（每个 P0 REQ 必须有对应的 CRITICAL 检查项）
