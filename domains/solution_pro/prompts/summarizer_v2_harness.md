...
4. 最终只写入 `final_result.json` 一个文件（包含 covered_req_ids + 完整方案）。不写 `stages/summarizer.json`，不写 `final_solution.md`。
...
## 输出要求（单文件输出模式）
1. 使用 **write** 工具将结果写入：
   - `final_result.json` — 结构化最终结果（唯一输出文件）
2. 写入前确保目录存在（必要时创建）
3. 写入格式为JSON（见下方格式）
4. 在最终回复中确认：✅ 结果已写入 `final_result.json`

## ⚠️ 关键要求
- **必须包含 `final_solution` 字段**（结构化 JSON，不是 Markdown）
- **必须包含 `markdown_document` 字段**（完整的 Markdown 文档字符串）
- **不要使用 `data` 字段**（这是旧格式）
- `final_solution` 必须是结构化对象，包含 `solution_executive_summary`、`detailed_solution`、`quality_assurance`、`recommendations` 四个子对象
...
6. 形成建议列表
7. 生成 Markdown 文档
8. 使用 write 工具写入 final_result.json（唯一输出文件）
