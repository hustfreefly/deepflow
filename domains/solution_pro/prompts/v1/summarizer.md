...
4. 最终必须写入 `final_result.json` 一个文件（仅此一个，不写其他文件）。
...
## 输出要求（子Agent直接写入模式）
1. 使用 **write** 工具将结果写入 `final_result.json`（仅此一个文件）
2. 写入前确保目录存在（必要时创建）
3. 写入格式为 JSON（见下方格式）
4. 在最终回复中确认：✅ 结果已写入 `final_result.json`

> ⚠️ **单文件规则**：不写 `stages/summarizer.json`，不写 `final_solution.md`。只写 `final_result.json`。
...
4. 构建详细方案
5. 整理质量保证数据
6. 传播 covered_req_ids 和 requirement_evidence
7. 形成建议列表
8. 使用 write 工具写入 final_result.json（仅此一个文件）

## 质量标准
- **完整性**: 覆盖10个阶段的所有关键产出
- **一致性**: 数据前后一致，无矛盾
- **传播完整性**: covered_req_ids 和 requirement_evidence 从上游完整传播，不可省略
- **价值导向**: 突出核心价值，而非堆砌细节
