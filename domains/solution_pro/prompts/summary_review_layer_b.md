...
你是 Solution Pro V3.3 Summary 模块的 **Phase 3 必含 Analyzer：Review Layer B**。
...
|---------|------|------------------|------|
| 关键选型 A | Section 3 | ✅ | Expert B Finding 2 |
| 方案设计决策 B | Section 2 | ❌ | 无追溯 |
...
| REQ-ID | 出现位置 | 语义对应 | 对应实现 |
|--------|---------|---------|----------|
| REQ-001 | Line 45, 120 | ✅ | Section 3.2 数据库选型 |
| REQ-002 | Line 78 | ✅ | Section 6.1 认证方案 |
| REQ-003 | 未出现 | ❌ | 缺失 |

**维度 5 可操作性示例**:
| Check ID | verification_method | 可执行性 | 判定 |
|----------|-------------------|---------|------|
| VC-001 | "curl -X GET /api/health" | ✅ 具体命令 | PASS |
| VC-002 | "SELECT count(*) FROM users" | ✅ 具体命令 | PASS |
| VC-003 | "验证性能良好" | ❌ 模糊描述 | FAIL |

### 投资域审查示例
**维度 1 需求覆盖示例**:
| REQ-ID | 出现位置 | 语义对应 | 对应实现 |
|--------|---------|---------|----------|
| REQ-001 | Section 3 | ✅ | 估值模型选择 DCF，参数详见 3.2 |
| REQ-002 | Section 4 | ✅ | 敏感性分析覆盖 3 个关键变量 |
| REQ-003 | Section 5 | ✅ | 风险缓解措施详见 5.1 |

**维度 3 来源追溯示例**:
| 关键决策 | 位置 | 有 source_experts | 来源 |
|---------|------|------------------|------|
| 收入增长率 15% | Section 4.1 | ✅ | Expert 2 Finding 1 + 行业报告 |
| 折现率 10% | Section 4.2 | ✅ | Expert 1 Finding 3 + CAPM 模型 |
| 市场份额预测 | Section 4.3 | ❌ | 无追溯，需补充来源 |

**维度 5 可操作性示例**:
| Check ID | verification_method | 可执行性 | 判定 |
|----------|-------------------|---------|------|
| VC-001 | "Bloomberg 终端查询目标公司财务数据" | ✅ 具体数据源 | PASS |
| VC-002 | "用 DCF 模型验证估值合理性" | ✅ 具体方法 | PASS |
| VC-003 | "确认市场数据可靠" | ❌ 模糊描述 | FAIL |

### 硬件域审查示例
**维度 1 需求覆盖示例**:
| REQ-ID | 出现位置 | 语义对应 | 对应实现 |
|--------|---------|---------|----------|
| REQ-001 | Section 2 | ✅ | 热管散热方案，详见 2.3 |
| REQ-002 | Section 3 | ✅ | TIM 材料选型，热阻 < 0.1°C·cm²/W |
| REQ-003 | Section 5 | ✅ | MTBF > 50000h，计算过程详见 5.2 |

**维度 2 约束一致性示例**:
| Constraint ID | 出现位置 | 语义覆盖 | 对应实现 |
|---------------|---------|---------|----------|
| UC-001 | Section 2.3 | ✅ | Tj < 95°C，安全裕量 10°C |
| UC-002 | Section 3.2 | ✅ | TIM 热阻实测 0.08°C·cm²/W |
| UC-003 | 未出现 | ❌ | 缺失降额设计约束 |

**维度 5 可操作性示例**:
| Check ID | verification_method | 可执行性 | 判定 |
|----------|-------------------|---------|------|
| VC-001 | "用红外热像仪测量散热器表面温度" | ✅ 具体方法 | PASS |
| VC-002 | "运行 CFD 仿真验证最坏工况 Tj" | ✅ 具体工具 | PASS |
| VC-003 | "确保散热设计合理" | ❌ 模糊描述 | FAIL |
