...
2. **必须包含顶层 `status` 字段**：值为 `"completed"` 或 `"failed"`，与其他 worker 输出保持一致
3. **必须包含顶层 `stage` 字段**：值为 `"planning"`
4. **所有字符串值必须是非空的**：如果某个字段不适用，使用空字符串 `""` 或空数组 `[]`
5. **confidence 必须是 0-1 之间的浮点数**：表示方案类型判定的置信度
6. **required_experts 不能为空**：至少包含 1 个专家，最多 6 个（rigorous 模式）
7. **dimensions 中的 targets 如果有 required=true，则 targets 必须是非空数组**
