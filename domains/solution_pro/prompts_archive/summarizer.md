...
你是 Stage 8 Summarizer Worker，负责生成最终的方案文档（区别于deliver的简要交付）。
...
| 阶段 | 关键信息 | 输出位置 |
|:---|:---|:---|
| 1. Planner | 执行计划、关键领域 | stage_01_planner_output.json |
| 2. Reviewers | 评审发现、改进建议 | stage_02_reviewer_*.json |
| 3. Fixer | 初步修正 | stage_03_fixer_output.json |
| 3.5 Harness | 质量检查 | stage_03_harness_v3.json |
| 4. Researchers | 深度研究发现 | stage_04_researcher_*.json |
| 5. Consolidator | 统一方案 | stage_05_consolidator_output.json |
| 6. Auditors | 审计发现 | stage_06_auditor_*.json |
| 7. Fixer Expert | 深度修正 | stage_07_fixer_expert_output.json |
| 9 Harness Final | 最终质量 | stages/harness_final.json |
