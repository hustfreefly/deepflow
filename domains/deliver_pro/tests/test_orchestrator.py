"""Tests for DeliverProOrchestrator."""
...
def orchestrator(wp, bb_path):
    return DeliverProOrchestrator(wp, bb_path, project_name="test_project")
...
class TestInit:
    def test_creates_directories(self, orchestrator, bb_path):
        assert (bb_path / "deliver_pro" / "data").is_dir()
        assert (bb_path / "deliver_pro" / "stages").is_dir()
        assert (bb_path / "deliver_pro" / "stages" / "worker_outputs").is_dir()

    def test_writes_wp_json(self, orchestrator, bb_path):
        wp_path = bb_path / "deliver_pro" / "data" / "wp.json"
    ...
    def test_wp_not_overwritten(self, wp, bb_path):
        ...
        orch1 = DeliverProOrchestrator(wp, bb_path)
        # Modify the file
        wp_path = bb_path / "deliver_pro" / "data" / "wp.json"
        wp_path.write_text('{"modified": true}', encoding="utf-8")

        orch2 = DeliverProOrchestrator(wp, bb_path)
        data = json.loads(wp_path.read_text(encoding="utf-8"))
...
class TestPrepareWorkersSpawn:
    ...
    def test_respects_parallelism(self, wp, bb_path):
        ...
        orch = DeliverProOrchestrator(wp, bb_path)
...
class TestGenerateTrackJson:
    ...
    def test_graceful_skip_when_extractor_unavailable(self, orchestrator):
        ...
        import domains.deliver_pro.orchestrator as orch_module
