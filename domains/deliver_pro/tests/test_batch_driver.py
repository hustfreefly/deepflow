"""
Tests for BatchDriver — 批量驱动 + Stuck Detection。
"""
...
@contextmanager
def _make_batch_driver(tmp_path, mock_blackboard):
    """创建 BatchDriver（mock BLACKBOARD_ROOT）。

    Usage:
        with _make_batch_driver(tmp_path, mock_blackboard) as driver:
    ...
    with patch("domains.deliver_pro.BLACKBOARD_ROOT", bb_root):
        ...
        driver = BatchDriver(project_name)
        yield driver
...
class TestTopoLayers:
    ...
    def test_no_dependencies_all_layer_0(self):
        ...
        layers = BatchDriver._topo_layers(wp_deps)
        assert len(layers) == 1
...
class TestCheckWpPhase:
    ...
    def test_pending_empty_dir(self, tmp_path, mock_blackboard):
        ...
        stages_dir = bb_root / wp_project / "deliver_pro" / "stages"
    ...
    def test_done_terminal_state(self, tmp_path, mock_blackboard):
        ...
        bb_root, project_name = mock_blackboard
        wp_project = "deliver_core_001"
        deliver_pro_dir = bb_root / wp_project / "deliver_pro"
    ...
    def test_not_done_without_manifest(self, tmp_path, mock_blackboard):
        ...
        bb_root, project_name = mock_blackboard
        wp_project = "deliver_core_001"
        final_dir = bb_root / wp_project / "deliver_pro" / "stages" / "final_deliverable"
        final_dir.mkdir(parents=True)
        (final_dir / "DELIVERABLE.md").write_text("# Done")
    ...
    def test_validating_integrated_draft_exists(self, tmp_path, mock_blackboard):
        ...
        bb_root, project_name = mock_blackboard
        wp_project = "deliver_core_001"
        draft_dir = bb_root / wp_project / "deliver_pro" / "stages" / "integrated_draft"
