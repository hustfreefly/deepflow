...
def inject_req_traceability(base_prompt: str, session_id: str) -> str:
    ...
    return base_prompt + "\n\n" + REQ_TRACEABILITY_INSTRUCTION.format(
        blackboard_path=f"{_DEEPFLOW_BASE}/blackboard/{session_id}"
    )
...
**关键约束**:
{chr(10).join([f"- {k}: {v}" for k, v in confirmed.get("constraints", {}).items()]) if confirmed.get("constraints") else "- 未指定"}

## 你的角色相关需求分组（Planner: Core + Functional）
...
def build_researcher_task(expert: str, session_id: str, topic: str, context: dict,
    ...
    prompt = prompt.replace("{{ topic }}", topic)
    prompt = prompt.replace("{{ constraints }}", json.dumps(layer2_constraints or {}, ensure_ascii=False, indent=2))
...
## 输出要求（子Agent直接写入模式）
1. 使用 **write** 工具将结果写入：
   `{_get_stage_path(session_id, "stages/research_")}{expert_id}.json`
2. 写入前确保目录存在（必要时创建）
3. 写入格式为JSON，包含以下字段：
   ```json
   {{
   ...
   ```
4. 在最终回复中确认：✅ 结果已写入 `{_get_stage_path(session_id, "stages/research_")}{expert_id}.json`
"""
    return prompt + "\n" + ctx
...
def build_auditor_task(session_id: str, topic: str, context: dict,
    ...
    prompt = prompt.replace("{{ TOPIC }}", topic)
    prompt = prompt.replace("{{ SOLUTION_TYPE }}", context.get("type", "architecture"))
    prompt = prompt.replace("{{ CONSTRAINTS }}", json.dumps(layer2_constraints or context.get("constraints", {}), ensure_ascii=False, indent=2))
...
def build_reviewer_task(session_id: str, topic: str, review_type: str,
    ...
    # 替换模板变量
    constraints_text = json.dumps(layer2_constraints or {}, ensure_ascii=False, indent=2)
...
"""
        ...
        # P5: 提取全局理解字段（why/for_whom/success_criteria）
        why = "\n".join([f"- {p}" for p in confirmed.get("pain_points", [])]) or "- 未指定"
...
4. 风险整合：合并所有识别的风险，去重"""
    ...
    prompt = prompt.replace("{{ topic }}", topic)
    prompt = prompt.replace("{{ quality_requirements }}", json.dumps(layer2_constraints or {}, ensure_ascii=False, indent=2))
