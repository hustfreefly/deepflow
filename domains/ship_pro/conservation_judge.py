...
# ============================================================================
# 自动修复（追加丢失的 anchors 到产物 metadata）
# ============================================================================

def auto_repair_lost_anchors(
    stage_output: Dict[str, Any],
    judge_result: Dict[str, Any],
    semantic_anchors: List[Dict[str, Any]],
) -> Dict[str, Any]:
    """
    自动修复：将丢失的 anchors 追加到产物的 metadata 中。
    
    不修改产物语义内容，只追加 metadata。
    这是"追加修复"，不是"重写修复"——安全性优先。
    """
    if judge_result["verdict"] == "PASS":
        return stage_output
    
    lost_names = {item.get("name") for item in judge_result.get("lost", [])}
    lost_anchors = [a for a in semantic_anchors if a.get("name") in lost_names]
    
    if not lost_anchors:
        return stage_output
    
    # 追加到 metadata
    if "_conservation_metadata" not in stage_output:
        stage_output["_conservation_metadata"] = {}
    
    stage_output["_conservation_metadata"]["inherited_constraints"] = lost_anchors
    stage_output["_conservation_metadata"]["conservation_note"] = (
        f"以下约束由 Conservation Judge 自动补入（alignment_rate={judge_result['alignment_rate']:.2f}）: "
        f"{[a['name'] for a in lost_anchors]}"
    )
    
    logger.info(
        f"自动修复: 补入 {len(lost_anchors)} 个丢失的 semantic anchors: "
        f"{[a['name'] for a in lost_anchors]}"
    )
