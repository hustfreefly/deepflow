...
if grep -q "updated_at: Optional\[str\]" domains/ship_pro/v6/orchestrator/state_manager.py; then
...
if grep -q "_extract_req_ids" domains/ship_pro/v6/contracts/gates.py; then
...
if grep -q "_extract_covered_req_ids" domains/ship_pro/v6/contracts/gates.py; then
...
if grep -q "execution_layers: List\[List\[str\]\]" domains/ship_pro/v6/contracts/ship_package.py; then
