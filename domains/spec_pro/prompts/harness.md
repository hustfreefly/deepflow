...
- 约束条件与功能需求是否兼容？
  ...
  - 投资域: "追求最高回报率" + "只投Pre-A轮以前的早期项目" + "要求2年内退出" → 三方矛盾
  - 硬件域: "成本控制在50元以内" + "必须用航空级材料" + "量产周期3个月" → 三方矛盾
  - 商业域: "客单价控制在30元以内" + "使用进口有机食材" + "6个月内回本" → 三方矛盾
- 质量属性之间是否兼容？
...
评估: 需求内容是否足够完整和结构化，适合下游引擎消费。

> 字段存在性已由代码 Layer 1 检查，你只需评估语义层面的完整度：需求描述是否足够详细？是否有遗漏的关键需求维度？路由建议是否合理？

检查:
- 需求描述是否足够详细，下游引擎无需再次猜测用户意图？
- solution_pro_hints 的 focus_areas 是否准确反映了需求的核心重点？
- route_recommendation 的 reasoning 是否与需求复杂度匹配？
