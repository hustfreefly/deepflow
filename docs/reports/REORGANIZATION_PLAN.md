...
| `core/agents/` | Agent 定义 | **加入契约**，保留 | 独立层 |
| `docs/cron/` | 定时任务 | **加入契约**，保留 | 独立层 |
| `config/pipelines/` | 流水线定义 | **加入契约**，保留 | 独立层 |
| `config/data/` | 数据文件 | 检查是否可归入 config/ | 需确认内容 |
| `config/data_sources/` | 数据源配置 | 合并到 `config/data_sources/` | 是配置 |
| `core/data_providers/` | 数据提供者 | 检查是否可合并 | 需确认内容 |
| `docs/reviews/` | 审查文档 | 移入 `docs/archive/` | 是文档 |
| `docs/research/` | 研究文档 | 移入 `docs/archive/` | 是文档 |
| `config/industries/` | 行业数据 | 检查是否可归入 config/ | 需确认内容 |
| `blackboard/checkpoints/` | 检查点 | 移入 `blackboard/` | 是运行时数据 |
| `blackboard/state/` | 状态文件 | 移入 `blackboard/` | 是运行时数据 |
| `blackboard/output/` | 输出文件 | **删除**（空目录） | 无内容 |
| `docs/audit_reports/` | 审计报告 | 移入 `docs/archive/` | 是文档 |
| `tests/results/` | 测试结果 | 移入 `tests/` | 是测试数据 |
| `docs/reports/` | 报告 | 检查是否可归入 docs/ | 需确认内容 |
