...
```
cage/active/[module]_v[X.Y].yaml     ← 每个模块一个契约文件
cage/archive/                         ← 已完成/过时的契约
```
...
> **注**：以内容完整性为优先，大小为参考。在契约文件中声明 `complexity` 字段自动应用对应限制。
...
```
Step 1: 写契约 YAML（cage/active/[module]_v[X.Y].yaml）
  └── 定义：接口 + 行为 + 边界
...
### 4.1 契约验证（scripts/validate.py）

```bash
# 验证单个模块契约
python scripts/validate.py config_loader
...
### 4.2 编码规范检查（scripts/check_standards.py）

```bash
# 检查单个模块
python scripts/check_standards.py config_loader.py
...
```
□ 契约文件存在（cage/active/[module]_v[X.Y].yaml）
...
| 模块 | 契约文件 | 大小 | Redlines |
|:---|:---|:---|:---|
| Spec Pro | `cage/active/spec_pro_v2.0.yaml` | 24KB | 8 |
| Research Pro | `cage/active/research_pro_v1.0.yaml` | 18KB | 13 |
| Investment | `cage/active/investment_v2.0.yaml` | 5.4KB | 8 |
| Solution Pro | `cage/active/solution_v1.0.yaml` | 9.7KB | 7 |

---

*契约笼子版本: 2.0.0 | 2026-05-30*  
