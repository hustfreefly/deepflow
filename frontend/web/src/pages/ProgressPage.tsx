...
const ProgressPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const checkStatus = async () => {
      try {
        const response = await fetch(`/api/v2/status/${sessionId}`)
        if (!response.ok) {
        ...
        // If completed, redirect to report after delay
        if (data.status === 'completed') {
          setTimeout(() => navigate(`/report/${sessionId}`), getPollingConfig().report_delay_ms)
        }
      } catch (err) {
    ...
    checkStatus()
    const interval = setInterval(checkStatus, getPollingConfig().task_ms)
    ...
    <div className="min-h-screen bg-surface-variant">
      {!online && (
        <div className="bg-red-50 border-b border-red-200 px-4 py-2 text-center">
          ...
          <span className="text-sm text-red-700">网络已断开，进度更新暂停</span>
        </div>
      )}
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-outline-variant mb-6">
              <div className="flex items-center justify-between mb-4">
                <div>
                  ...
                  {status.current_stage && status.current_stage !== 'waiting' && (
                    <p className="text-sm text-onsurface-variant">
                      当前阶段: {stageNames[status.current_stage] || status.current_stage}
                    </p>
                  )}
                  <p className="text-sm text-onsurface-variant">
                ...
                <div className="text-right">
                  <span className={`text-3xl font-bold ${
                    ...
                    status.status === 'queued' || status.current_stage === 'waiting' ? 'text-yellow-600' : 'text-primary-600'
                  }`}>
                    {status.current_stage === 'waiting' ? '排队中' :
              ...
              <div className="w-full h-3 bg-surface-variant rounded-full overflow-hidden">
                <div
                  className={`h-full rounded-full transition-all duration-500 ${
                    status.status === 'queued' || status.current_stage === 'waiting' ? 'bg-yellow-400' : 'bg-primary-500'
                  }`}
                  style={{ width: `${status.progress * 100}%` }}
                ></div>
              </div>

              {(status.status === 'queued' || status.current_stage === 'waiting') && (
                <p className="text-sm text-yellow-600 mt-3 flex items-center gap-1">
                  ...
                  任务已提交,等待 Cron Job 自动触发执行(约 2 分钟内)...
                </p>
              )}
            </div>
