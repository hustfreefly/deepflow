...
# 更新引用（如有）
grep -rl "reviews/" --include="*.py" --include="*.yaml" --include="*.md" . | xargs sed -i '' 's|reviews/|docs/reviews/|g'
...
grep -rl "pipelines/" --include="*.py" --include="*.yaml" --include="*.md" . \
...
sed -i '' 's|config: "data_sources/|config: "config/data_sources/|g' domains/investment.yaml

# 4.1b: 更新所有 Python/MD/YAML 中的路径引用
grep -rl "data_sources/" --include="*.py" --include="*.yaml" --include="*.md" . \
...
import os
assert os.path.exists('config/data_sources/investment.yaml'), '文件不存在'
print('✅ config/data_sources/investment.yaml 存在')
"
...
| 文件 | 引用方式 | 修改内容 |
|------|---------|---------|
| domains/investment/__init__.py | `from data_providers.investment import register_providers` | → `from core.data_providers.investment import register_providers` |
| core/data_manager_worker.py | `from data_providers.investment import register_providers` | → `from core.data_providers.investment import register_providers` |
| data_providers/investment.py | `from data_providers.tushare_provider import TushareProvider` | → `from core.data_providers.tushare_provider import TushareProvider` |
| scripts/data_collect_smic.py | `from data_providers.investment import register_providers` | → `from core.data_providers.investment import register_providers` |
...
# 5.1c: core/data_providers/investment.py（内部引用）
sed -i '' 's|from data_providers\.|from core.data_providers.|g' core/data_providers/investment.py
...
sed -i '' 's|"data_providers/|"core/data_providers/|g' tests/unit/check_data_manager.py
...
# 验证 3: 旧路径不存在
import os
assert not os.path.exists('data_providers/'), '旧目录仍存在'
print('✅ 旧目录已移除')

# 验证 4: 新路径存在
assert os.path.exists('core/data_providers/investment.py'), '新文件不存在'
print('✅ 新文件存在')
...
| 文件 | 引用方式 | 修改内容 |
|------|---------|---------|
| frontend/backend/routers/tasks_v2.py | 硬编码字符串 `from agents.webhook_task_processor` | → `from core.agents.webhook_task_processor` |
| frontend/backend/routers/consumer.py | 硬编码字符串 `from agents.webhook_task_processor` | → `from core.agents.webhook_task_processor` |
| tests/test_e2e_webhook.py | `from agents.webhook_task_processor import ...` | → `from core.agents.webhook_task_processor import ...` |
| agents/cron_task_checker.py | 内部引用 `from agents.webhook_task_processor` | → `from core.agents.webhook_task_processor` |
| cage/check_wh003.py | `from agents.webhook_task_processor import ...` | → `from core.agents.webhook_task_processor import ...` |
| 4个 cage/*.yaml | 路径引用 `agents/` | → `core/agents/` |
...
# 验证 3: 旧路径不存在
assert not os.path.exists('agents/'), '旧目录仍存在'
print('✅ 旧目录已移除')
...
assert 'from agents.' not in content or 'core.agents' in content, '旧引用仍存在'
print('✅ 前端代码引用已更新')
