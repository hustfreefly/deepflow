...
- ✅ **硬编码引用**（动态生成 Python 代码）：
  - `frontend/backend/routers/tasks_v2.py:213` - 生成 webhook 任务时硬编码 `from agents.webhook_task_processor`
  - `frontend/backend/routers/consumer.py:74` - 生成 consumer 任务时硬编码 `from agents.webhook_task_processor`
...
- ✅ **Python import**（核心业务逻辑）：
  - `domains/investment/__init__.py` - 多次 import `from data_providers.investment import register_providers`
  - `core/data_manager_worker.py` - import `from data_providers.investment import register_providers`
  - `scripts/data_collect_smic.py` - import `from data_providers.investment import register_providers`
  - `ARCHIVED/v1.0_legacy/orchestrator_agent.py` - import（历史代码）
  - `data_providers/investment.py` 内部 import `from data_providers.tushare_provider import TushareProvider`
...
**引用情况：**
- ✅ **配置引用**：
  - `domains/investment.yaml` - `config: "data_sources/investment.yaml"`
  - `domains/investment/__init__.py` - 多处文档引用 `data_sources/investment.yaml`
...
**引用情况：**
- `core/search_engine.py` 提到 `data/search_config.yaml`，但这是注释
- **没有硬编码路径**
...
### 阶段 0：删除空目录（2分钟，零风险）
- 删除 `checkpoints/`、`state/`、`output/`

### 阶段 1：移动低风险目录（15分钟，零风险）
- 移动 `pipelines/` → `config/pipelines/`
- 移动 `industries/` → `config/industries/`
- 移动 `data/` → `config/data/`
- 移动文档类目录到 `docs/`
- 移动 `test_results/` → `tests/results/`
